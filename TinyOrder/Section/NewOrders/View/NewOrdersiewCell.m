...
- (void)createSubView:(CGRect)frame mealCoutn:(int)mealCount
{
    if (!_orderView) {
        ...
        UIView * menuLineView = [[UIView alloc] initWithFrame:CGRectMake(SPACE, _orderView.bottom + MENUVIEW_HEIGHT * mealCount, VIEW_WIDTH, 1.5)];
...
//        _remarkLabel.backgroundColor = [UIColor orangeColor];
        ...
        UIView * lineView = [[UIView alloc] initWithFrame:CGRectMake(SPACE, _remarkLabel.bottom, _remarkLabel.width, 1.5)];
...
- (void)setOrderModel:(NewOrderModel *)orderModel
{
    ...
    self.orderView.addressLabel.text = orderModel.address;
    self.orderView.telLabel.text = orderModel.tel;
    self.orderView.contactsLabel.text = orderModel.contect;
    for (int i = 0; i < orderModel.mealArray.count; i++) {
        ...
        NSLog(@"%d", orderModel.mealArray.count);
    }
    ...
    if ([orderModel.pey boolValue]) {
        self.totalView.stateLabel.text = @"已付款";
        self.totalView.stateImageV.image = [UIImage imageNamed:@"state@2x.png"];
    }else
    {
//        self.totalView.stateLabel.text = @"未付款";
//        self.totalView.stateImageV.image = [UIImage imageNamed:@""];
    }
}
