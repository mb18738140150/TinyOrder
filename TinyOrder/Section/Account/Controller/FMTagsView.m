...
- (instancetype)initWithName:(NSString *)name MealID:(NSNumber *)mealId font:(UIFont *)font;
...
- (void)calculateContentSize
{
    ...
    CGSize textSize = [_name boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, 1000) options:NSStringDrawingUsesLineFragmentOrigin attributes:dict context:nil].size;
    _contentSize = CGSizeMake(ceil(textSize.width + 10), ceil(textSize.height));
}
@end
...
- (void)layoutSubviews
{
    ...
    self.tagLabel.frame = CGRectMake(0, 0, width - 10, [self.tagModel contentSize].height);
    self.backImageview.center = self.contentView.center;
...
- (void)prepareLayout
{
    ...
    CGFloat minimumInteritemSpacing = [self minimumLineSpacingAtSection:0];
...
@property (strong, nonatomic) NSMutableArray<MenuActivityMD *> *tagsMutableArray;
...
- (void)commonInit
{
    ...
    _contentInserts = UIEdgeInsetsMake(10, 10, 10, 10);
    ...
    NSArray * constraints = [NSLayoutConstraint constraintsWithVisualFormat:@"H:|[collectionView]|" options:NSLayoutFormatAlignAllTop metrics:nil views:views];
...
- (void)setTagsArray:(NSArray<MenuActivityMD *> *)tagsArray {
    _tagsMutableArray = [tagsArray mutableCopy];
    [self.tagModels removeAllObjects];
    
    for (MenuActivityMD * model in tagsArray) {
        FMTagModel * tagModel = [[FMTagModel alloc]initWithName:model.name MealID:model.mealId font:self.tagFont];
...
//    }];
    ...
    for (NSInteger idx = 0; idx < self.tagsMutableArray.count; idx++) {
        MenuActivityMD * model = [self.tagsMutableArray objectAtIndex:idx];
        if ([model.name isEqualToString:tagName]) {
            index = idx;
            break;
        }
    }
...
- (NSArray<MenuActivityMD *> *)tagsArray {
